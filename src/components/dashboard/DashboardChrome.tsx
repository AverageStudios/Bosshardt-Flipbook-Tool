 ...
 */
export function DashboardChrome({ folders, children }: { folders: Folder[]; children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-dvh lg:flex">
      <aside className="sticky top-0 hidden h-dvh w-60 shrink-0 border-r border-line bg-surface lg:block">
        <DashboardSidebar folders={folders} />
      </aside>
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          >
            ...
            <DashboardSidebar folders={folders} onNavigate={() => setDrawerOpen(false)} />
          </div>
        </div>
      )}
